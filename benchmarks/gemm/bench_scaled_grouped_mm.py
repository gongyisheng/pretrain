...
def _quant(a, b, scheme):
    ...
    aq, sa, _, _ = quantize_operand(a, -1, fmt, _ROWWISE)
    bq, sb, _, _ = quantize_operand(b, -2, fmt, _ROWWISE)  # (E,K,N) in one call
...
def _bench_wgrad_point(E, K, N, scheme):
    ...
    fmt = _fmt(scheme)
    a = a.mT
    aq, sa, _, _ = quantize_operand(a, -1, fmt, _ROWWISE, offs=offs, ragged_dim=-1)
    gq, sg, _, _ = quantize_operand(g, -2, fmt, _ROWWISE, offs=offs, ragged_dim=-2)
    ref = _scaled_grouped_mm(
