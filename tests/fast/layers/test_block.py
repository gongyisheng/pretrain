...
class _MinimalTransformerBlock(BaseTransformerBlock):
    ...
    def __init__(self, d_model: int, n_heads: int, intermediate_size: int, dropout_attn: float = 0.0, dropout_ffn: float = 0.0, attn_implementation: str = "sdpa"):
        super().__init__(d_model)
        self.ln1 = nn.LayerNorm(d_model)
        self.attn = MultiHeadAttention(d_model, n_heads, dropout_attn, attn_implementation=attn_implementation)
