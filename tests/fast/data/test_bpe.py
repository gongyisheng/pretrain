...
# ---- Pre-encoded chunks path (equivalence with corpus_iter path) ----


def test_bpe_train_requires_exactly_one_input():
    """train() needs corpus_iter XOR chunks — neither or both is an error."""
    trainer = BpeTrainer(vocab_size=300, n_workers=1)
    with pytest.raises(ValueError, match="exactly one"):
        trainer.train()  # type: ignore[call-arg]
    with pytest.raises(ValueError, match="exactly one"):
        trainer.train(_corpus, chunks={("a",): 1})


def test_bpe_chunks_param_equivalent_no_resume():
    """Fresh train via raw corpus vs pre-built chunks yields identical output.

    The chunks dict mirrors what `_build_chunks(mode='bytelevel', n_workers=1)`
    would produce internally — feeding it directly should be a no-op.
    """
    # Path A: corpus_iter, BpeTrainer pretokenizes internally
    v_a, m_a = BpeTrainer(vocab_size=400, pretokenizer="bytelevel", n_workers=1).train(
        _corpus
    )

    # Path B: pre-build the same chunks and pass them directly
    chunks = _build_chunks(_corpus, mode="bytelevel", n_workers=1)
    v_b, m_b = BpeTrainer(vocab_size=400, pretokenizer="bytelevel", n_workers=1).train(
        chunks=chunks
    )
...
def test_bpe_chunks_param_equivalent_with_resume():
    """The load-bearing test for the SuperBPE superword pass optimization.

    Path A (current production): resume from a subword vocab/merges and pass
    raw byte-level chunks — BpeTrainer replays subword merges in Python via
    `_init_pair_state`.
    Path B (optimized): pre-encode each doc with the saved subword tokenizer
    (HF Rust encoder) and pass the resulting tuples as `chunks`. BpeTrainer
    skips the replay because chunks already reflect the subword merges.

    If this test passes, the two paths are semantically equivalent and the
    HF-encoded shortcut is safe to wire into `_train_superbpe`.
    """
    from tokenizers import Tokenizer, models, pre_tokenizers

    # Train a small subword pass first.
    subword_v, subword_m = BpeTrainer(
        vocab_size=300, pretokenizer="bpe", n_workers=1
    ).train(_corpus)

    # Path A: raw corpus, BpeTrainer replays subword merges internally.
    v_a, m_a = BpeTrainer(
        vocab_size=400,
        pretokenizer="bytelevel",
        initial_vocab=subword_v,
        initial_merges=subword_m,
        n_workers=1,
    ).train(_corpus)

    # Path B: encode each doc with the subword tokenizer (same use_regex=False
    # config as the saved subword_tokenizer.json), then pass as chunks.
    subword_tok = Tokenizer(models.BPE(vocab=subword_v, merges=subword_m))
    subword_tok.pre_tokenizer = pre_tokenizers.ByteLevel(
        add_prefix_space=False, use_regex=False
    )
    pre_encoded: Counter = Counter()
    for doc in _corpus():
        enc = subword_tok.encode(doc, add_special_tokens=False)
        pre_encoded[tuple(enc.tokens)] += 1
