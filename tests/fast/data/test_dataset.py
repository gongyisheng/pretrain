...
def packed_docs(mock_memmap):
    ...
    mock_memmap["/docs.bin"] = np.array([2, 3, EOT, 4, 5, 6, EOT, 7, EOT], dtype=np.uint16)
    return "/docs.bin"


def test_single_doc_length(packed_docs):
    ds = PretrainDataset(packed_docs, seq_len=8, vocab_size=256, packing=False, eot_token_id=EOT, pad_token_id=PAD)
    assert len(ds) == 3
...
def test_single_doc_truncation(mock_memmap):
    ...
    ds = PretrainDataset("/long.bin", seq_len=4, vocab_size=256, packing=False, eot_token_id=EOT, pad_token_id=PAD)
    input_ids, position_ids = ds[0]
