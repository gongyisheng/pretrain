...
def test_operand_a_scale_shape(gran, bs, nkb):
    x = torch.randn(8, 256)
    xq, s = quantize_operand(x, -1, _E4M3, _scaling(gran, bs))
...
def test_operand_b_scale_shape(gran, bs, nkb):
    ...
    xq, s = quantize_operand(x, -2, _E4M3, _scaling(gran, bs))
...
def test_blockwise_partial_last_block():
    ...
    x = torch.randn(4, 100)
    xq, s = quantize_operand(x, -1, _E4M3, _scaling("blockwise", 32))
...
def test_rowwise_fp8_roundtrips_within_error():
    ...
    fq = _roundtrip(x, -1, _E4M3, _scaling("rowwise"))
...
def test_tensorwise_fp8_roundtrips_within_error():
    ...
    xq, _ = quantize_operand(x, -1, _E4M3, _scaling("tensorwise"))
    assert xq.dtype == torch.float8_e4m3fn
    recon = _roundtrip(x, -1, _E4M3, _scaling("tensorwise"))
...
def test_zero_input_safe(gran, bs):
    x = torch.zeros(4, 64)
    fq = _roundtrip(x, -1, _E4M3, _scaling(gran, bs))
...
def test_int_quant_rounds_and_clamps():
    ...
    xq, s = quantize_operand(x, -1, "int4", _scaling("rowwise"))
...
def test_int_tensorwise_symmetric_range_and_scale(fmt):
    ...
    qmax = str_to_qmax(fmt)
    x = torch.randn(64, 64) * 10.0
    q, s = quantize_operand(x, -1, fmt, _scaling("tensorwise"))
...
def test_int_tensorwise_recon_within_half_scale(fmt):
    ...
    recon = _roundtrip(x, -1, fmt, _scaling("tensorwise"))
...
def test_fp32_scale_never_clips(granularity):
    ...
    fq = _roundtrip(x, -1, _E4M3, _scaling(granularity))
...
def _manual_dequant(x, contract_dim, bs, fmt, scale_dtype=torch.float32):
    ...
    xq, s = quantize_operand(
        x, contract_dim, fmt, _scaling("blockwise", bs, scale_dtype)
    )
...
def test_dequantize_operand_preserves_shape_and_layout(gran, bs, contract_dim):
    torch.manual_seed(0)
    x = torch.randn(64, 96)
    xq, s = quantize_operand(x, contract_dim, _E4M3, _scaling(gran, bs))
    deq = dequantize_operand(xq, s, contract_dim, _scaling(gran, bs))
...
def test_dequantize_operand_returns_float32_from_bf16():
    ...
    xq, s = quantize_operand(x, -1, _E4M3, _scaling("rowwise"))
    assert dequantize_operand(xq, s, -1, _scaling("rowwise")).dtype == torch.float32
...
def test_dequantize_operand_stacked_experts_matches_per_expert(gran, bs):
    ...
    per_expert = [
        quantize_operand(experts[g], -2, _E4M3, _scaling(gran, bs))
    ...
    reference = torch.stack(
        [
            _roundtrip(experts[g], -2, _E4M3, _scaling(gran, bs)).float()
            for g in range(experts.shape[0])
        ]
    )
...
def test_dequantize_operand_partial_block_uses_given_block_size():
    ...
    torch.manual_seed(0)
    x = torch.randn(4, 100)
    scaling = _scaling("blockwise", 32)
    xq, s = quantize_operand(x, -1, _E4M3, scaling)
    deq = dequantize_operand(xq, s, -1, scaling)
...
def test_dequantize_operand_rejects_a_contraction_outside_the_last_two_dims():
    ...
    with pytest.raises(ValueError, match="dim must be -2 or -1"):
        dequantize_operand(xq, scale, 0, _scaling("tensorwise"))
...
def test_quantize_operand_rejects_a_contraction_outside_the_last_two_dims():
    with pytest.raises(ValueError, match="contract_dim must be -2 or -1"):
        quantize_operand(torch.randn(4, 8), 0, _E4M3, _scaling("tensorwise"))


def test_quantize_operand_rejects_offs_without_ragged_dim():
    with pytest.raises(ValueError, match="together"):
        quantize_operand(
            torch.randn(4, 8), -1, _E4M3, _scaling("tensorwise"), offs=_offs([2, 2])
        )
...
def test_ragged_quantize_cold_group_scale_ignores_hot_group():
    ...
    _, scale = quantize_operand(
        x, -2, _E4M3, _scaling("rowwise"), offs=_offs([4, 4]), ragged_dim=-2
    )
...
def test_ragged_quantize_matches_independent_per_group_quantize(gran, bs):
    ...
    offs = _offs(counts)
    xq, _ = quantize_operand(x, -2, _E4M3, _scaling(gran, bs), offs=offs, ragged_dim=-2)
    lo = 0
    for hi in offs.tolist():
        if hi > lo:
            # a single group is a non-ragged tensor: the existing code path
            expected, _ = quantize_operand(x[lo:hi], -2, _E4M3, _scaling(gran, bs))
...
def test_ragged_none_is_unchanged(contract_dim, gran, bs):
    ...
    torch.manual_seed(0)
    x = torch.randn(64, 96)
    xq, scale = quantize_operand(x, contract_dim, _E4M3, _scaling(gran, bs))
    assert xq.shape == x.shape
...
def test_ragged_dequantize_inverts_ragged_quantize(gran, bs):
    ...
    xq, scale = quantize_operand(x, -2, _E4M3, scaling, offs=offs, ragged_dim=-2)
    deq = dequantize_operand(xq, scale, -2, scaling, offs=offs, ragged_dim=-2)
...
def test_ragged_tensorwise_row_scale_is_per_group():
    x = torch.cat([torch.full((4, 3), 100.0), torch.full((4, 3), 1.0)])
    xq, scale = quantize_operand(
        x, -1, _E4M3, _scaling("tensorwise"), offs=_offs([4, 4]), ragged_dim=-2
    )
...
def test_ragged_ignored_for_already_per_row_granularities(gran, bs):
    ...
    with_ragged = quantize_operand(
        x, -1, _E4M3, _scaling(gran, bs), offs=_offs([4, 4]), ragged_dim=-2
    )
