...
# --- gemm impl: scaled kernel vs dequantize-then-matmul ---


@fp8_only
def test_gemm_impl_dequant_takes_the_dequant_branch():
    torch.manual_seed(0)
    a = torch.randn(64, 256, device="cuda", dtype=torch.bfloat16)
    b = torch.randn(256, 96, device="cuda", dtype=torch.bfloat16)
    # mirror the dequant branch exactly: dequantize, cast back, then matmul
    exact = (
        _roundtrip(a, -1, "tensorwise", 0, "fp8_e4m3").to(a.dtype)
        @ _roundtrip(b, 0, "tensorwise", 0, "fp8_e4m3").to(b.dtype)
    ).to(torch.float32)

    deq, _, _ = quantized_gemm(
        a, b, "fp8_e4m3", "fp8_e4m3", torch.float32, {}, impl="dequant"
    )
    scaled, _, _ = quantized_gemm(
        a, b, "fp8_e4m3", "fp8_e4m3", torch.float32, {}, impl="scaled"
    )
    # same quantized operands either way, so the dequant path reproduces the
    # reference exactly while the kernel accumulates differently
    assert torch.equal(deq, exact)
    assert not torch.equal(scaled, exact)


@fp8_only
def test_gemm_impl_defaults_to_scaled():
    torch.manual_seed(0)
    a = torch.randn(64, 256, device="cuda", dtype=torch.bfloat16)
    b = torch.randn(256, 96, device="cuda", dtype=torch.bfloat16)
    default, _, _ = quantized_gemm(a, b, "fp8_e4m3", "fp8_e4m3", torch.float32, {})
    scaled, _, _ = quantized_gemm(
        a, b, "fp8_e4m3", "fp8_e4m3", torch.float32, {}, impl="scaled"
    )
    assert torch.equal(default, scaled)
...
def test_gemm_key_scopes_to_its_own_gemm():
    ...
    def run(gemm):
        ...
        q = QuantizedLinear.from_module(
            lin, QuantizationConfig(enabled=True, dtype={"recipe": "fp8"}, gemm=gemm)
        )
        x = torch.randn(
            64, 256, device="cuda", dtype=torch.bfloat16, requires_grad=True
        )
        torch.manual_seed(1)
        q(x).backward(torch.randn(64, 128, device="cuda", dtype=torch.bfloat16))
        return x.grad.clone(), q.weight.grad.clone()

    dx_all, dw_all = run("dequant")
    dx_none, dw_none = run("scaled")
    dx_w, dw_w = run({"wgrad": "dequant"})

    assert torch.equal(dw_w, dw_all)  # wgrad followed the dequant path
    assert not torch.equal(dw_w, dw_none)
    assert torch.equal(dx_w, dx_none)  # dgrad stayed on the kernel
    assert not torch.equal(dx_w, dx_all)
