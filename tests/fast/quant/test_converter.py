...
DTYPE_RECIPE_CASES = [
    # (recipe, resolved per-tensor / per-GEMM dtypes)
    ...
    *[
    ...
    (
        "mxfp8",  # e4m3 on the gradient too, since the block scale carries the range
        {
            "weight": {"fwd": "fp8_e4m3", "dgrad": "fp8_e4m3"},
            "act": {"fwd": "fp8_e4m3", "wgrad": "fp8_e4m3"},
...
@pytest.mark.parametrize("recipe,expected", DTYPE_RECIPE_CASES)
def test_quantization_config_dtype_recipe(recipe, expected):
    assert QuantizationConfig(enabled=True, dtype={"recipe": recipe}).dtype == expected
