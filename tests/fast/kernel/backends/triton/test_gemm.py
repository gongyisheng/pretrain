...
def test_scaled_mm_precision(case, format, scaling, with_bias, out_dtype):
    ...
    aq, bq, sa, sb, out_dtype, block_size, bias = make_scaled_mm_inputs(
        case, format, scaling, with_bias=with_bias, dtype=out_dtype
    )
...
@pytest.mark.parametrize("format", QUANT_FORMAT_CASES, ids=lambda case: case.name)
@pytest.mark.parametrize("scaling", QUANT_SCALE_ERROR_CASES, ids=lambda case: case.name)
@pytest.mark.parametrize("with_bias", BIAS_CASES, ids=["no-bias", "bias"])
def test_scaled_mm_raise_error(case, format, scaling, with_bias):
    ...
    """
    aq, bq, sa, sb, out_dtype, block_size, bias = make_scaled_mm_inputs(
        case, format, scaling, with_bias=with_bias
    )
...
@pytest.mark.parametrize("scaling", MXFP8_SCALE_CASES, ids=lambda case: case.name)
@pytest.mark.parametrize("with_bias", BIAS_CASES, ids=["no-bias", "bias"])
@pytest.mark.parametrize(
...
def test_mxfp8_scaled_mm_precision(
    case, format, scaling, with_bias, out_dtype, global_rel_tol
):
...
@pytest.mark.parametrize("case", SCALED_MM_ERROR_CASES, ids=lambda case: case.name)
@pytest.mark.parametrize("scaling", MXFP8_SCALE_ERROR_CASES, ids=lambda case: case.name)
@pytest.mark.parametrize("with_bias", BIAS_CASES, ids=["no-bias", "bias"])
def test_mxfp8_scaled_mm_raise_error(case, scaling, with_bias):
    ...
    """
    aq, bq, sa, sb, out_dtype, block_size, bias = make_scaled_mm_inputs(
        case, FP8_E4M3_FORMAT, scaling, with_bias=with_bias
    )
...
def test_scaled_grouped_mm_precision(
    case, layout, format, scaling, out_dtype, with_bias
):
    capability = torch.cuda.get_device_capability()
    if format.a_format == "int8" and capability < (8, 0):
...
def test_mxfp8_scaled_grouped_mm_precision(
    case, layout, format, scaling, with_bias, out_dtype, global_rel_tol
):
...
@pytest.mark.parametrize("format", QUANT_FORMAT_CASES, ids=lambda case: case.name)
@pytest.mark.parametrize("scaling", QUANT_SCALE_ERROR_CASES, ids=lambda case: case.name)
def test_scaled_grouped_mm_rejects_unsupported_scale_block_width(
    layout, format, scaling
):
...
@pytest.mark.parametrize(
    "scaling", [BLOCKWISE1D_32_SCALING, BLOCKWISE1D_64_SCALING, BLOCKWISE1D_128_SCALING]
)
...
@pytest.mark.parametrize(
    ("layout", "scaling"),
    [("ragged_m", TENSORWISE_SCALING), ("ragged_k", BLOCKWISE1D_64_SCALING)],
