...
SWIZZLE_DTYPES = (torch.float8_e4m3fn, torch.float8_e8m0fnu)
...
@pytest.mark.parametrize("dtype", SWIZZLE_DTYPES)
def test_to_swizzle_32_4_4_pads_and_maps_logical_scale_bytes(rows, blocks, dtype):
    scale = (
        torch.tensor(
            [2.0 ** (index % 10 - 5) for index in range(rows * blocks)],
            dtype=torch.float32,
        )
        .reshape(rows, blocks)
        .to(dtype)
    ...
    logical = scale.to(dtype).view(torch.uint8)
    actual_bytes = actual.view(torch.uint8)
    zero_byte = torch.tensor(0.0, dtype=dtype).view(torch.uint8).item()
...
@pytest.mark.parametrize("dtype", UNSUPPORTED_SWIZZLE_DTYPES)
def test_to_swizzle_32_4_4_raise_error(dtype):
    scale = torch.ones((1, 1), dtype=dtype)

    with pytest.raises(ValueError, match="float8_e4m3fn or float8_e8m0fnu"):
        to_swizzle_32_4_4(scale)
