...
`src/utils/config.py` owns config defaults and validation: `ModelConfig.__post_init__` fills resolved kwargs and rejects invalid combinations. Components receive explicit resolved values. Norm, positional embedding, residual, and the optimizer use `*_cls` plus `*_kwargs`; attention and MLP are per-layer `{*_cls, *_kwargs, layer_idx?}` lists. `optimizer_kwargs` holds the selected optimizer class's own constructor kwargs and is splatted into it verbatim (`lr`, `lr_mult`, `weight_decay` stay top-level); keys left unset fall back to that class's signature defaults, so only values that differ from them belong in YAML. One unscoped entry applies to every unclaimed layer; scoped entries override named layers, and at most one unscoped fallback is allowed. `resolve_attn(i)` and `resolve_mlp(i)` are the source of per-layer resolution.
