...
All FP8 configs explicitly set tensorwise scales with `block_shape: [0, 0]` for weight, activation, and `grad_out`. Quantization applies to eligible attention and MLP linear layers; embeddings, lm_head, norms, attention, and optimizer state remain bf16/fp32.
...
- Related experiments: `fp8_granularity/` and `int8_weight_granularity/`.
