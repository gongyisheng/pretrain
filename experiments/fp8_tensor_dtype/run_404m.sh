...
#   nohup env CUDA_VISIBLE_DEVICES=0 bash -c 'cd "'"$PWD"'"; c=qwen3_404m_fp8_e4m3_w8a8;   echo "=== $c $(date)"; uv run python scripts/train.py --config experiments/fp8_tensor_dtype/$c.yaml' > logs/fp8_tensor_dtype_404m_gpu0.log 2>&1 &
#   nohup env CUDA_VISIBLE_DEVICES=1 bash -c 'cd "'"$PWD"'"; c=qwen3_404m_fp8_e4m3_w8a8g8; echo "=== $c $(date)"; uv run python scripts/train.py --config experiments/fp8_tensor_dtype/$c.yaml' > logs/fp8_tensor_dtype_404m_gpu1.log 2>&1 &
#   nohup env CUDA_VISIBLE_DEVICES=0 bash -c 'cd "'"$PWD"'"; c=qwen3_404m_fp8_e4m3_w8a8_e5m2_g8; echo "=== $c $(date)"; uv run python scripts/train.py --config experiments/fp8_tensor_dtype/$c.yaml' > logs/fp8_tensor_dtype_404m_gpu4.log 2>&1 &
