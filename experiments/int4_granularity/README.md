...
- **W4A4G4** additionally quantizes `grad_out`, putting int4 in both backward GEMMs. Gradients have the widest dynamic range of the three tensors, and int4 is fixed-point with only 16 codes, so a scale set by the largest gradient element flushes almost the whole distribution to zero. Expect this arm to diverge at most or all granularities; the interesting question is whether `(1, 16)` is enough to keep it training at all.
...
**Granularity axis** (identical in all three arms). Effective bits/weight counts fp32 scale storage against the weight tensor only; the W4A4 and W4A4G4 arms additionally carry activation and gradient scales, which are transient and not counted here. int4 sets a 4-bit code range (`qmax=7`) but the tensor container is int8, so this column is a logical accounting, not measured memory.
...
|---|---|---|---|---|---|
| W4A16 | int4 | bf16 | bf16 | dequantize → bf16 matmul (simulated) | dequantize → bf16 matmul (simulated) |
| W4A4 | int4 | int4 | bf16 | fused `gemm.int8_scaled_mm` (int8 container) | dequantize → bf16 matmul (simulated) |
| W4A4G4 | int4 | int4 | int4 | fused `gemm.int8_scaled_mm` (int8 container) | fused `gemm.int8_scaled_mm` (int8 container) |
...
### W4A4G4 (int4 weights, activations, and `grad_out`)

| Granularity | `block_shape` | Effective bits/weight | Val Loss | Δ vs bf16 | Δ vs W4A4 twin | Val BPB | Weight rel. err |
|---|---|---|---|---|---|---|---|
| blockwise1D | (1, 64) | 4.50 | | | | | |
| blockwise1D | (1, 32) | 5.00 | | | | | |
| blockwise1D | (1, 16) | 6.00 | | | | | |
...
`Δ vs W4A4 twin` isolates the cost of quantizing `grad_out` to int4 at that granularity. Divergence is the expected outcome for most of this arm — record the step it occurred rather than a final loss.

### 1D vs 2D at matched extent

| Extent | W4A16 (1, N) | W4A16 (N, N) | Δ | W4A4 (1, N) | W4A4 (N, N) | Δ | W4A4G4 (1, N) | W4A4G4 (N, N) | Δ |
|---|---|---|---|---|---|---|---|---|---|
| 16 | | | | | | | | | |
| 32 | | | | | | | | | |
| 64 | | | | | | | | | |
...
- `weight rel. err` should fall as blocks shrink; compare it with validation loss to isolate quantization error. It is an arm-independent weight property, so it should match across each W4A16/W4A4/W4A4G4 triple.
- Record instability, divergence, and loss spikes, not just final-window loss. Coarse-block int4 — especially W4A4G4, and W4A4 at `(64, 64)` — is the most likely cell in the quant experiments to diverge outright; note the step if it does instead of reporting only a final number.
