...
W8A8 runs additionally bound the dense MLP pre-activation with `mlp_kwargs.activation_limit` set to the code max of the activation dtype (int8→127, int7→63, int6→31, int5→15, int4→7), as in gpt-oss's `swiglu_limit` and DeepSeek V4.
...
Tying the activation limit to the dtype max shrinks the pre-activation range in step with the shrinking code range, so outliers can't stretch the activation scale as the bit width drops. This should help most where the scale is shared widest — tensorwise, low bits — and cost the least where there are codes to spare.
...
| qwen3_51m_int8_w8a8 | int8 | int8 | bf16 | 127 |
| qwen3_51m_int7_w8a16 | int7 | bf16 | bf16 | — |
| qwen3_51m_int7_w8a8 | int7 | int7 | bf16 | 63 |
| qwen3_51m_int6_w8a16 | int6 | bf16 | bf16 | — |
| qwen3_51m_int6_w8a8 | int6 | int6 | bf16 | 31 |
| qwen3_51m_int5_w8a16 | int5 | bf16 | bf16 | — |
| qwen3_51m_int5_w8a8 | int5 | int5 | bf16 | 15 |
| qwen3_51m_int4_w8a16 | int4 | bf16 | bf16 | — |
| qwen3_51m_int4_w8a8 | int4 | int4 | bf16 | 7 |
...
| Config | Granularity | `activation_limit` | Final Val Loss | Δ vs bf16 |
|---|---|---|---|---|
| qwen3_51m_bf16 | — | — | | 0 |
| qwen3_51m_int8_w8a16_rowwise | rowwise | — | | |
| qwen3_51m_int8_w8a8_rowwise | rowwise | 127 | | |
| qwen3_51m_int7_w8a16_rowwise | rowwise | — | | |
| qwen3_51m_int7_w8a8_rowwise | rowwise | 63 | | |
| qwen3_51m_int6_w8a16_rowwise | rowwise | — | | |
| qwen3_51m_int6_w8a8_rowwise | rowwise | 31 | | |
| qwen3_51m_int5_w8a16_rowwise | rowwise | — | | |
| qwen3_51m_int5_w8a8_rowwise | rowwise | 15 | | |
| qwen3_51m_int4_w8a16_rowwise | rowwise | — | | |
| qwen3_51m_int4_w8a8_rowwise | rowwise | 7 | | |
| qwen3_51m_int8_w8a16_tensorwise | tensorwise | — | | |
| qwen3_51m_int8_w8a8_tensorwise | tensorwise | 127 | | |
| qwen3_51m_int7_w8a16_tensorwise | tensorwise | — | | |
| qwen3_51m_int7_w8a8_tensorwise | tensorwise | 63 | | |
| qwen3_51m_int6_w8a16_tensorwise | tensorwise | — | | |
| qwen3_51m_int6_w8a8_tensorwise | tensorwise | 31 | | |
| qwen3_51m_int5_w8a16_tensorwise | tensorwise | — | | |
| qwen3_51m_int5_w8a8_tensorwise | tensorwise | 15 | | |
| qwen3_51m_int4_w8a16_tensorwise | tensorwise | — | | |
| qwen3_51m_int4_w8a8_tensorwise | tensorwise | 7 | | |
...
- The activation limit is an absolute pre-activation magnitude that happens to equal the dtype's code max; it is not applied in code space and does not by itself force one activation per code. It only touches the dense MLP's `gate_up_proj` output, so it bounds the `down_proj` input but not attention activations.
- W8A8 cells therefore move two knobs at once. [`int8_activation_limit`](../int8_activation_limit/README.md) isolates the limit at fixed int8 tensorwise, and [`activation_limit`](../activation_limit/README.md) gives its bf16-only cost.
