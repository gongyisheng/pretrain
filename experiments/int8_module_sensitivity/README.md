...
This mirrors `experiments/fp8_module_sensitivity/` one-for-one — same architecture, schedule, optimizer, seed, module groups, and `tensorwise` scaling. Only the number format changes, so the two Δ tables are directly comparable.
...
- **MLP** (`gate_up_proj`, `down_proj`) — `down_proj`'s input is the SwiGLU product, the canonical outlier-heavy activation. Expected most int8-sensitive, and a module fp8 barely notices.
...
| qwen3_77m_int8_attn | attention | `[blocks.*.attn.*]` | 32 (4×8) |
| qwen3_77m_int8_mlp | MLP | `[blocks.*.mlp.*]` | 16 (2×8) |
| qwen3_77m_int8_lm_head | output head | `[lm_head]` | 1 |
