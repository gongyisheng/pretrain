...
- **e4m3_w8a8_e5m2_g8** — weight/act `fp8_e4m3`, `grad_out` `fp8_e5m2` in both backward GEMMs. The standard mixed-format setup: e5m2 trades mantissa for exponent range on the wider-dynamic-range gradients.
- **e4m3_w8a8g8** — every operand (weight/act/grad) `fp8_e4m3`. Keeps grad mantissa precision at the cost of dynamic range; stresses whether the extra range in e5m2 actually matters here.

Both are written as explicit `dtype` maps so the grad-format axis is visible in the config.
...
| qwen3_51m_fp8_e4m3_w8a8_e5m2_g8_adamw | AdamW | fp8 tensorwise | e5m2  | 5e-4 | 5e-5 | 0.1 | 256 | ~51M |
| qwen3_51m_fp8_e4m3_w8a8_e5m2_g8_muon  | Muon  | fp8 tensorwise | e5m2  | 5e-4 | 5e-5 | 0.1 | 256 | ~51M |
| qwen3_51m_fp8_e4m3_w8a8g8_adamw       | AdamW | fp8 tensorwise | e4m3  | 5e-4 | 5e-5 | 0.1 | 256 | ~51M |
| qwen3_51m_fp8_e4m3_w8a8g8_muon        | Muon  | fp8 tensorwise | e4m3  | 5e-4 | 5e-5 | 0.1 | 256 | ~51M |
...
The script runs the bf16 baselines first, then sweeps `{e4m3_w8a8_e5m2_g8, e4m3_w8a8g8} × {adamw, muon}`.
...
| 51M | AdamW | fp8 e4m3 W8A8 + e5m2 G8 | TBD | TBD | — |
| 51M | Muon  | fp8 e4m3 W8A8 + e5m2 G8 | TBD | TBD | TBD |
| 51M | AdamW | fp8 e4m3 W8A8G8          | TBD | TBD | — |
| 51M | Muon  | fp8 e4m3 W8A8G8          | TBD | TBD | TBD |
...
- `e4m3_w8a8_e5m2_g8` vs `e4m3_w8a8g8` isolates the grad format: if `e4m3_w8a8g8` degrades (or diverges), e5m2's extra dynamic range on gradients is load-bearing at tensorwise scaling; if it matches `e4m3_w8a8_e5m2_g8`, the grad mantissa precision wins the trade. Watch whether the effect differs between AdamW and Muon.
