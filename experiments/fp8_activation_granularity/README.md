...
Five runs: three W8A8 activation granularities, one BF16 baseline, and one W8A16 weight-only control. All quantized runs use fixed 32×32 blockwise FP8 E4M3 weights; W8A8 runs also use FP8 E4M3 activations. They use FP32 scales, round-to-nearest-even, unquantized BF16 `grad_out`, and exclude `lm_head`. No activation clipping or Hadamard rotation is enabled.

| Config (`.yaml`) | Weight | Activation block | Forward values per activation scale |
|---|---|---|---|
| `qwen3_51m_bf16` | BF16 | BF16 | — |
| `qwen3_51m_fp8_w8a16` | FP8 E4M3 (32, 32) | BF16 | — |
| `qwen3_51m_fp8_w8a8_act_rowwise` | FP8 E4M3 (32, 32) | FP8 E4M3 rowwise | 512 / 1,536 (projection-dependent) |
| `qwen3_51m_fp8_w8a8_act_blockwise1d_32` | FP8 E4M3 (32, 32) | FP8 E4M3 (1, 32) | 32 |
| `qwen3_51m_fp8_w8a8_act_blockwise2d_32` | FP8 E4M3 (32, 32) | FP8 E4M3 (32, 32) | 1,024 |
...
Batch size 128 and accumulation 2 intentionally match the INT8 activation-granularity experiment.
...
mkdir -p logs
nohup bash experiments/fp8_activation_granularity/run.sh > logs/fp8_activation_granularity.log 2>&1 &
```

The launcher runs all five configurations sequentially on the selected device. W&B project: `pretrain-fp8-activation-granularity`. Checkpoints: `checkpoints/fp8_activation_granularity/<config>/`.
...
- **Arithmetic paths:** rowwise FP8 activations have contraction extents that mismatch the 32×32 FP8 weight tiles, so forward quantizes and dequantizes both operands before the BF16 matmul fallback. The 1D and 2D blockwise activations both have a 32-element contraction extent and use the scaled FP8 forward path. W8A16 forward and all backward GEMMs run in BF16 after any FP8 operands are quantized and dequantized; `grad_out` remains unquantized. Do not use this experiment for pure timing or scale-only numerical comparisons across the activation granularities.
- Fixed square weight tiles keep the weight policy identical across the sweep and its transposes. Learned weights and their quantization errors can still diverge across runs.
