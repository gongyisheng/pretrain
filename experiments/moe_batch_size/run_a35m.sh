#!/bin/bash
# Batch-size sweep on qwen3_133m_a35m (top-2): fix batch_size=8, sweep gradient
...
# Usage: nohup bash experiments/moe_batch_size/run_a35m.sh > logs/moe_batch_size_a35m.log 2>&1 &
...
for ga in "${GRAD_ACCUM[@]}"; do
    config="experiments/moe_batch_size/qwen3_133m_a35m_ga${ga}.yaml"
    echo "=== ${config} ==="
